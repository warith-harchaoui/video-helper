...
- Edges carry video *and metadata* (width × height, duration, framerate,
  ...
  `overlay`).
- One toggle per node: **"Bypass"**, which routes the input straight to
  the output. Makes A/B trivial: compare the graph with a node bypassed
  vs active.
...
- `A / B` keys: a 1-frame dial between two versions, the kind mixing
  consoles actually use.
...
- **Colorblind-safe by construction.** All state uses shape + color +
  text, never color alone (see companion `sprezzature-colors` audit skill).
...
- Front end: TypeScript + Svelte 5 + Vega-Lite (metadata plots) +
  ...
  scrubbing. No React: matches the `sprezzature-ui` companion skill's stack.
